    ...
    # Structure To Tackle Operators And Aliases
        ...
        for i in range(index, len(expression)):
            ...
            if (expression[i] in all_operators) or (expression[i].isalpha()):
                ...
                raise Exception(f'Invalid operator "{expression[index:i+2]}"')
            else:
                index = i
                break
        else:
...
def coordinate(final_expression):
    try:
        parenthesis = pre_parse(final_expression)
        return eval(parenthesis)
    except Exception as e:
        return f'Error: {e}' 


print(coordinate('32+4*(4+2'))                            # Error: Not matching parenthesis
print(coordinate('3/(2-2)'))                            # Error: Division by zero
print(coordinate('2 mul 3 pow 2 div 3'))                # 6.0
print(coordinate('2 mul 3 pow (2 divv 3)'))             # Error: Invalid operator "divv"
print(coordinate('test + 3'))                           # Error: Invalid operator "test+"
print(coordinate('5 +@! (-6.3)'))                         # Error: Invalid operator "+@!"